...
struct actdat *actdat_get(const char* base)
{
    ...
    actdat = (struct actdat *) malloc(sizeof(struct actdat));
	if (actdat == NULL)
		goto fail;

    snprintf(path, sizeof(path), "%s" "act.dat", base);

	std::vector<char> data = FS::Load(path);
	memcpy(actdat, data.data(), data.size());
...
int rap2rif(const uint8_t* rap, const char* content_id, const char *exdata_path)
{
	...
	LOG("actdat_get path=%s", exdata_path);
	...
	rif.expiration = 0;
	rif.accountid = Util::BE64(actdat->accountId);
	...
	if (!FS::Save(path, (uint8_t*) &rif, sizeof(struct rif))) {
